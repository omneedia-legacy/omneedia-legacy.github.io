...
var TFilterBoxAdd = function(p,s) {
			...
			launch.on('click',function() {
				...
				for (var i=0;i<p.items.items.length;i++) {
					...
					if (i>0) {
						if (filter.items.items[0].getText()=="ET") ff.operator=" AND "; else ff.operator=" OR ";
					};
					objs.push(ff);
				};
				console.log(objs);
						...
						click: function(x,e) {
							...
							*/
							if (x._operand=="date") {
								...
								if (Ext.getVersion().major >= 5) panel.updateLayout(); else panel.doLayout();
			...
			if (Ext.getVersion().major >= 5) p.updateLayout(); else p.doLayout();			
};
