...
App.apply(App, {
    ...
    /**
     * Returns namespace.
     * @param {String} value The className
     * @return {NS}
     */
    namespace: function (cs) {
        //return this.createNamespace(cs);
    },
    /**
	...
	*/
     ...
     * Defines a class
     */
    define: function (className, o, createdFn) {
        // We create statics objects
        /*
                if (!App.global.classes) App.global.classes = [];
                if (!App.global._class) App.global._class = {};
                if (App.global.classes.indexOf(className) > -1) {
                    if (createdFn) createdFn(App.global._class[className]);
                    return App.global._class[className];
                };
        */
        function init() {
            if (o.statics) {
            ...
            //App.global.classes.push(className);
            //App.global._class[className] = object;
            if (createdFn) createdFn(object);
            return object;
        };
        if (o.require) App.require(o.require, init);
        else init();
    },
    logger: {
        oldConsoleLog: null,
        ...
        if (Settings.DEBUG) {
            ...
            for (var el in Settings.PATHS) {
                if (el.indexOf('@') > -1) {
                    ...
                    script = script.replace(re, Settings.PATHS[el]);
                }
            };
